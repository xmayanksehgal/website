...
class SkillManager extends Model
{
    ...
    public function findAllForDump(){
        ...
        $query = $this->client->run($cyp);
        $resultSet = $query->getRecords();
     ...
     */
    public function findChildren($uuid='5430126ba0a292f58144425'){
        $cyp = "MATCH (parent:Skill)-[:HAS]->(s:Skill)
                        WHERE parent.uuid = {uuid}
                        RETURN s
                        ORDER BY s.created ASC LIMIT 40";
        $query = $this->client->run($cyp,["uuid" => $uuid]);
        $resultSet = $query->getRecords();
        var_dump($resultSet);
...
//        $query = $query = $this->client->run($cyp, array("depth" => $depth));
//        $resultSet = $query->getRecords();
//
//        return $resultSet;
//    }
...
//        $query = $query = $this->client->run($cyp, array("uuid" => $skill->getUuid()));
//        $resultSet = $query->getRecords();
//
//        $revisions = array();
//        foreach($resultSet as $row){
...
//        $query = $query = $this->client->run($cyp, array(
//                "uuid"          => $uuid)
//        );
//        $resultSet = $query->getRecords();
...
//        }
//
//        $query = $query = $this->client->run($cyp, array("keywords" => $regexp));
...
//                    LIMIT 1";
//
//        $query = $query = $this->client->run($cyp, array("uuid" => $uuid));
//
//        $resultSet = $query->getRecords();
//        if ($resultSet->count() > 0){
//            $row = $resultSet[0];
...
//
//        $query = $query = $this->client->run($cyp, $namedParams);
//        $resultSet = $query->getRecords();
//
...
//        $query = $query = $this->client->run($cyp);
//        $query->getResultSet();
//    }
//
//
...
//
//        $query = $query = $this->client->run($cyp, array("userUuid" => $user->getUuid()));
//        $resultSet = $query->getRecords();
//
//        $activities = array();
//        if ($resultSet->count() > 0){
